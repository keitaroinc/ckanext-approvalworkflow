...
def check_dataset_quality(key, data, errors, context):
    """
    Validator that prevents non-admin users from making datasets public
    unless the dataset has been approved by an admin.
    ...
    """
    log.info("=" * 60)
    log.info("check_dataset_quality validator called")
    log.info("=" * 60)
    ...
    # Get the approval_workflow field
    approval_key = ('approval_workflow',)
    approval_status = data.get(approval_key, '')
    
    log.info(f"Current approval status: '{approval_status}'")
    ...
    if is_org_admin:
        log.info("User IS admin - auto-approving dataset")
        # Auto-approve when admin makes it public
        data[approval_key] = 'approved'
        log.info("Set approval_workflow to 'approved'")
        return
    else:
        # User is NOT admin
        ...
        if approval_status == 'approved':
            log.info("Dataset already approved - allowing public")
            return
        else:
            log.warning(f"Dataset not approved (status: '{approval_status}') and user is not admin")
            errors[key].append(
